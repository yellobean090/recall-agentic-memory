...
import { executeTool, toOpenAIToolConfig } from "./tools.js";
...
export async function runAgentTurn(history) {
    ...
    while (message.tool_calls && message.tool_calls.length > 0) {
        ...
        for (const call of message.tool_calls) {
            ...
            toolTrace.push({ name: call.function.name, input, result });
            messages.push({
                role: "tool",
    ...
    const conversation = [...history, { role: "assistant", content: text }];
